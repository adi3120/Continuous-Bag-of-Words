...
def prepare_one_hot_inputs(vocabulary):
  inputs={}
  for i,j in enumerate(vocabulary):
    inputs[j]=np.zeros(len(vocabulary))
    inputs[j][i]=1
  return inputs

def prepare_outputs(matrix):
  outputs={}
  for word in matrix.keys():
      outputs[word]=matrix[word]
  return outputs

def co_matrix_probabilities(matrix, corpus):
    for word in matrix.keys():
        total_word_occurrences = countOccurrences(corpus, word)
        for context in matrix[word].keys():
            matrix[word][context] = matrix[word][context] / total_word_occurrences
    return matrix
...
co_matrix=co_matrix_prepare(corpus,vocabulary,2)
co_matrix=co_matrix_probabilities(co_matrix,corpus)

inputs=prepare_one_hot_inputs(vocabulary)
outputs=prepare_outputs(co_matrix)
...
def gradient_descent_CBOW_epoch(ANN, inputs, outputs, eta, epochs):
    ...
    for j in range(0, epochs):
        ...
        for key in inputs.keys():
            ...
            ANN[len(ANN) - 1].set_actual(outputs[key].values)

            for layer in ANN:
                layer.forward()

            actual_label = np.argmax(outputs[key].values)
            output = ANN[-1].output()
