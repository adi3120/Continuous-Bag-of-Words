...
def Binary_CrossEntropy_Loss(activations, actual):
    ...
    activations = np.clip(
        activations, epsilon, 1 - epsilon
    )  # Avoiding logarithm of zero
    loss = -np.sum(
        actual * np.log(activations) + (1 - actual) * np.log(1 - activations)
    )
...
class InputLayer:
    ...
    def put_values(self, values):
        if len(values) == self.length:
            ...
            print(
                "Error: The values you are trying to insert are more then the allocated size of input vector"
            )
    ...
    def set_weights(self, method="random"):
        if self.length > 0:
            if method == "normal_random":
                ...
                self.W = (1 / self.length**0.5) * np.random.randn(
                    self.length, self.previous.length
                )
            elif method == "one":
                self.W = np.ones((self.length, self.previous.length))
            elif method == "he":
                self.W = np.random.randn(self.length, self.previous.length) * np.sqrt(
                    2.0 / (self.previous.length * self.length)
                )
            elif method == "lecun":
                limit = np.sqrt(1.0 / self.previous.length)
                self.W = np.random.uniform(
                    -limit, limit, (self.length, self.previous.length)
                )
    ...
    def set_biases(self, method="random"):
        if self.length > 0 and self.previous != None:
            if method == "normal_random":
                ...
                self.Bias = np.random.randn(1, self.length) * np.sqrt(
                    1 / self.previous.length
                )
            # Add other bias initialization methods here...
...
def gradient_descent_epoch(ANN, x, y, eta, epochs):
    ...
    for j in range(0, epochs):
        ...
        loss.append(ANN[len(ANN) - 1].loss())
        print(
            f"epoch: {j}, Loss: {ANN[len(ANN)-1].loss()}, Accuracy: {100*corrects/len(y)}"
        )
    return ANN, loss


def gradient_descent_epoch_minibatch(ANN, x, y, eta, epochs, batch_size):
    loss = []
    batch_completed = 0
    for j in range(0, epochs):
        weight_gradient=[]
